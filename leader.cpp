...
leader::leader(vector<int>& P_leader, int leader_id)
    : P_leader(P_leader), leader_id(leader_id) {
        leader_send = vector<vector<vector<vector<int>>>>(10, vector<vector<vector<int>>>(10, vector<vector<int>>(10)));
        leader_recv = vector<vector<vector<int>>>(10, vector<vector<int>>(10));
    }
...
map<int, vector<tuple<int, int, int>>> leader::create_and_query(const vector<vector<int>>& random_vector, int L) {
    ...
    for (int i = 0; i < M; i++) {
        ...
        int counter = 0;
        int eta = ceil(R / (N[i] - 1));
        for (int j = 0; j < eta; j++) {
            auto rv = random_vector[j];
            leader_send[i][0][j] = rv;
            for (int k = 1; k < N[i]; k++) {
